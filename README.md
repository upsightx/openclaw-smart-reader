...
- **四级智能降级策略**：
  1. **直连抓取**：针对普通网页，直接提取核心内容。
  2. **长风盒子/Jina AI**：利用专业解析接口处理高难度排版。
  3. **Kedou 自动化引擎**：针对微信等强防护平台，利用浏览器自动化技术实现 100% 模拟人类操作，突破反爬封锁。
...
| `modules/kedou_parser.py` | 终极兜底引擎。通过 `agent-browser` 注入 JavaScript 调用 Kedou 核心 API 或模拟交互。 |

## 🛠️ 技术架构

- **自动化底座**：基于 `agent-browser` (Playwright/Chrome) 实现无头浏览器自动化。
- **API 逆向**：深度逆向 Kedou 解析站点的网络请求，实现了 bypass Cloudflare WAF 的"浏览器代理"模式。
